package live.uwapps.mpvkt.ui.player
...
import live.uwapps.mpvkt.databinding.PlayerLayoutBinding
import live.uwapps.mpvkt.domain.library.UpdateWatchProgressUseCase
...
import live.uwapps.mpvkt.ui.player.controls.PlayerControls
...
class PlayerActivity : AppCompatActivity() {
  ...
  private val updateWatchProgressUseCase: UpdateWatchProgressUseCase by inject()
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setupAudio()
    setupMediaSession()
    currentVideoPath = intent.getStringExtra("uri") ?: parsePathFromIntent(intent)
  ...
  private fun updateLibraryWatchProgress() {
    val path = currentVideoPath ?: return
    val posSec = viewModel.pos ?: return
    val durationSec = viewModel.duration ?: return
    if (durationSec <= 0) return
    val progress = (posSec.toFloat() / durationSec.toFloat()).coerceIn(0f, 1f)
    lifecycleScope.launch(Dispatchers.IO) {
      try {
        updateWatchProgressUseCase(path, progress)
      } catch (_: Exception) {}
    }
  }
  ...
  companion object {
    ...
    private const val RESULT_INTENT = "live.uwapps.mpvkt.ui.player.PlayerActivity.result"
  }
}
