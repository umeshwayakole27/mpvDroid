package com.uw.mpvDroid.ui.player
...
import com.uw.mpvDroid.databinding.PlayerLayoutBinding
...
import com.uw.mpvDroid.ui.player.controls.PlayerControls
...
class PlayerActivity : AppCompatActivity() {
  ...
  private fun getPlayableUri(intent: Intent): String? {
    ...
    Log.d(TAG, "getPlayableUri: uri=$uri, scheme=${intent.data?.scheme}")
  ...
  override fun onDestroy() {
    ...
    try {
      ...
      Log.e(TAG, "Error removing observer: ${e.message}")
    }
  ...
  private fun copyMPVScripts() {
    val mpvdroidLua = assets.open("mpvdroid.lua")
    val applicationPath = filesDir.path
    ...
    File("$scriptsDir/mpvdroid.lua")
      .also { if (!it.exists()) it.createNewFile() }
      .writeText(mpvdroidLua.bufferedReader().readText())
  }
  ...
  private fun saveVideoPlaybackState(mediaTitle: String) {
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      ...
      val timeRemaining = if (duration > 0) (duration - pos).coerceAtLeast(0) else 0
  ...
  companion object {
    ...
    private const val RESULT_INTENT = "com.uw.mpvDroid.ui.player.PlayerActivity.result"
  }
}
